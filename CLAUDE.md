...
| `GET /api/client/events/{eventID}/my-registration-status` (the caller's own registration sub-state for that event — `{"status":"none"\|"pending"\|"approved"\|"rejected"}`, unlike the collapsed approved-only check round eligibility uses) | access token |
